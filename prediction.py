...
class ClusterPrediction(ABC):
    ...
    def fit(self, train: pd.DataFrame, test: pd.DataFrame, target: str, fit_params=None) -> 'ClusterPrediction':
        fit_params = fit_params or dict()
        clusters = self.cluster_obj.fit_predict(X=pd.concat([train.drop(columns=target), test]))
...
class ClusterProbabilityPrediction(ClusterPrediction):
    ...
    def predict(self, test: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(self.n_clusters):
            ...
            indx_clusters.append((cluster.index, prediction))
        nclasses = indx_clusters[0][1].shape[1]
